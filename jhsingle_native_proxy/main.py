...
def make_app(destport, prefix, command, presentation_path, mappath, authtype, request_timeout, ready_check_path, ready_timeout, repo,
    ...
    proxy_handler = _make_serverproxy_handler('mainprocess', command, {}, False, destport, ready_check_path, ready_timeout, gitwrapper, mappath)
...
@click.option('--mappath', nargs=2, multiple=True, help='Regex substitution to map URL paths, requires a pattern and replacement')
...
def run(port, destport, ip, presentation_path, mappath, debug, logs, authtype, request_timeout, last_activity_interval, force_alive, ready_check_path,
    ...
    app = make_app(destport, prefix, list(command), presentation_path, mappath, authtype, request_timeout, ready_check_path,
